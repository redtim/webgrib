...
export function* walkMessages(buf, opts = {}) {
    ...
    while (reader.remaining >= 16 && emitted < limit) {
        ...
        if (section1Num !== 1)
            throw new Error(`Expected section 1, got ${section1Num}`);
        const section1 = parseSection1(reader, section1Len - 5);
        ...
        if (end !== '7777') {
            if (!opts.tolerant)
                throw new Error(`Expected 7777 end marker, got "${end}"`);
        }
