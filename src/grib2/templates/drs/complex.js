...
function parseComplexHeader(body, withSpatialDiff) {
 ...
 */
function finalizeValues(out, groupRef, groupLen, groupWidth, packed, packedBitOffset, h) {
 ...
 */
export function unpackComplex(s5, s6, s7, totalPoints) {
 ...
 * seed values.
 */
export function unpackComplexSpatial(s5, s6, s7, totalPoints) {
    return unpackComplexInternal(s5, s6, s7, totalPoints, true);
}
function unpackComplexInternal(s5, s6, s7, totalPoints, withSpatialDiff) {
    ...
    const groupRef = new Int32Array(h.numberOfGroups);
    {
        ...
        for (let g = 0; g < h.numberOfGroups; g++)
            groupRef[g] = br.readBits(h.nbits);
        br.align();
