 ...
 */
export function unpackSimple(s5: Section5, s6: Section6, s7: Section7, totalPoints: number): Float32Array {
  const header = parseSimplePackingHeader(s5.body);
  const count = s5.numberOfValues;

  let dense: Float32Array;
  if (header.bitsPerValue === 0) {
    ...
    for (let i = 0; i < count; i++) ints[i] = br.readBits(header.bitsPerValue);
