 ...
 */
export function ieeeFloat32(buf: Uint8Array, offset: number): number {
  const view = new DataView(buf.buffer, buf.byteOffset + offset, 4);
  return view.getFloat32(0, false);
}

export interface SimplePackingHeader {
  referenceValue: number; // R — IEEE32 float
  binaryScale: number;    // E — signed int16
  decimalScale: number;   // D — signed int16
  bitsPerValue: number;   // nbits — uint8
  typeOfOriginalFieldValues: number; // uint8 — 0 = float, 1 = int
}
 ...
 */
export function parseSimplePackingHeader(body: Uint8Array, offset = 0): SimplePackingHeader {
 ...
 */
export function simpleDequantize(
  out: Float32Array,
  ints: Uint32Array | Uint16Array | Uint8Array | number[],
  header: SimplePackingHeader,
): void {
