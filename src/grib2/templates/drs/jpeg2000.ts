...
export interface Jpeg2000Decoded {
  ...
  height: number;
  /**
   * Samples in row-major order, one element per pixel. For multi-component
   * codestreams we only look at component 0 (GRIB2 5.40 always uses a single
   * component).
   */
  samples: Int32Array | Uint32Array | Float32Array;
}

export type Jpeg2000Decoder = (bytes: Uint8Array) => Promise<Jpeg2000Decoded> | Jpeg2000Decoded;

let decoder: Jpeg2000Decoder | null = null;
export function setJpeg2000Decoder(d: Jpeg2000Decoder | null): void {
  decoder = d;
}
export function getJpeg2000Decoder(): Jpeg2000Decoder | null {
  return decoder;
}

export async function unpackJpeg2000(
  s5: Section5,
  s6: Section6,
  s7: Section7,
  totalPoints: number,
  ...
  if (!decoder) {
    throw new Error(
      'No JPEG2000 decoder registered. Call setJpeg2000Decoder() before decoding template 5.40 messages (e.g., the vendored jpx.js from PDF.js).',
    );
  }
  ...
  if (header.bitsPerValue === 0) {
    ...
    if (s6.bitmapIndicator === 0 && s6.bitmap) return applyBitmap(dense, s6.bitmap, count);
...
function padOrTruncate(dense: Float32Array, totalPoints: number): Float32Array {
  if (dense.length === totalPoints) return dense;
