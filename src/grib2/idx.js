...
export function findRecord(records, q) {
...
export async function fetchIdx(idxUrl, opts = {}) {
    const res = await fetch(idxUrl, { signal: opts.signal, headers: opts.headers });
    if (!res.ok)
        throw new Error(`Failed to fetch idx ${idxUrl}: ${res.status}`);
 ...
 */
export async function fetchMessageBytes(idxUrl, query, opts = {}) {
 ...
 */
export function forecastQuery(fhour) {
    return fhour === 0 ? /^anl$/ : new RegExp(`^${fhour} hour fcst$`);
}
export function hrrrUrls(cycle, fhour, product = 'wrfsfcf') {
