...
export function enableFloatTextureExtensions(gl: WebGL2RenderingContext): void {
...
export function createScalarTexture(gl: WebGL2RenderingContext, width: number, height: number, data: Float32Array): FloatTexture {
...
export function createVectorTexture(gl: WebGL2RenderingContext, width: number, height: number, uv: Float32Array): FloatTexture {
  const tex = gl.createTexture();
  if (!tex) throw new Error('createTexture failed');
  gl.bindTexture(gl.TEXTURE_2D, tex);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
...
export function createColormapTexture(gl: WebGL2RenderingContext, lut: Uint8Array): FloatTexture {
  const tex = gl.createTexture();
  if (!tex) throw new Error('createTexture failed');
  gl.bindTexture(gl.TEXTURE_2D, tex);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
