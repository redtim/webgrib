...
export function compileShader(gl, type, source, label) {
    ...
    if (!sh)
        throw new Error(`glCreateShader failed for ${label}`);
...
export function linkProgram(gl, vs, fs, label, transformFeedback) {
    const prog = gl.createProgram();
    if (!prog)
        throw new Error(`glCreateProgram failed for ${label}`);
...
export function buildProgram(gl, vertexSource, fragmentSource, label = 'program', transformFeedback) {
