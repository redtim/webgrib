/*  Global class for simulating the movement of particle through a 1km wind grid
 ...
 --------------------------------------------------------------------------
 Vendored into gribwebview from leaflet-velocity v1.10.x
   source:  https://github.com/onaci/leaflet-velocity/blob/master/src/js/windy.js
   license: CSIRO Open Source Software Licence (BSD/MIT variant) — see
            ./LICENSE-windy.txt
   changes: (1) exported as an ES module default export instead of a
                window global,
            (2) the `project` / `invert` coordinate transforms no longer
                reference Leaflet — they are supplied by the host via
                `params.project(lat, lon) -> [x, y]` and
                `params.invert(x, y) -> [lon, lat]` closures,
            (3) the window.cancelAnimationFrame polyfill at the bottom of
                the original file was removed (all target environments
                already support it).
 --------------------------------------------------------------------------
 */
...
export default function Windy(params) {
  ...
  // --- gribwebview patch ---------------------------------------------------
  // Original leaflet-velocity implementation referenced `params.map` (a
  // Leaflet map) and the `L.point` / `L.latLng` constructors. We instead take
  // host-provided closures so this file has no Leaflet dependency.
  var invert = function(x, y, windy) {
    return params.invert(x, y);
  };

  var project = function(lat, lon, windy) {
    return params.project(lat, lon);
  };
  // --- end gribwebview patch -----------------------------------------------
