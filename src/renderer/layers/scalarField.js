...
import { createColormapTexture, createScalarTexture, enableFloatTextureExtensions, } from '../gl/texture.js';
import { colormap } from '../colormaps.js';
import { LCC_GLSL, computeLccUniforms, gridLonLatBounds, lonLatToGridUV, lonLatToMercator } from '../projections/lcc.js';
...
export class ScalarFieldLayer {
    ...
    constructor(opts = {}) {
        ...
        if (opts.valueRange)
            this.valueRange = opts.valueRange;
    }
    onAdd(map, glAny) {
        const gl = glAny;
        this.map = map;
    ...
    setData(field, grid) {
        if (!this.gl || !this.program)
            throw new Error('ScalarFieldLayer.setData called before onAdd');
        const gl = this.gl;
        if (this.fieldTex)
            gl.deleteTexture(this.fieldTex.tex);
    ...
    setOpacity(op) { this.opacity = op; this.map?.triggerRepaint(); }
    setValueRange(range) { this.valueRange = range; this.map?.triggerRepaint(); }
    setVisible(visible) {
    ...
    render(glAny, matrix) {
        ...
        if (!this.visible)
            return;
        if (!this.program || !this.fieldTex || !this.lutTex || !this.lcc || !this.indexCount)
            return;
    ...
    onRemove(_map, glAny) {
        ...
        if (this.fieldTex)
            gl.deleteTexture(this.fieldTex.tex);
        if (this.lutTex)
            gl.deleteTexture(this.lutTex.tex);
