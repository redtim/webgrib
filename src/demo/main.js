...
const WIND_RANGE_MS = [0, 60 * KT_TO_MS];
...
async function main() {
    ...
    const findInsertionPoint = () => {
        ...
        for (const sl of ['transportation', 'boundary', 'place']) {
            const found = layers.find((l) => l['source-layer'] === sl);
            if (found)
                return found.id;
        }
        return undefined;
    ...
    for (const layer of map.getStyle().layers ?? []) {
        ...
        if (!sl || !sl.startsWith('transportation'))
            continue;
        if (layer.type === 'line') {
    ...
    const ltCheckbox = document.getElementById('toggle-lightning');
    ...
    async function loadLevel(variable, levelIndex, cycle, fhour) {
        if (loading)
            return;
        loading = true;
        currentVariable = variable;
        ...
        try {
            if (variable.kind === 'scalar' && level.query) {
                ...
                const { u, v, grid } = await client.decodePair(urls.idx, { parameter: level.queryU.parameter, level: level.queryU.level, forecast: fcRe }, { parameter: level.queryV.parameter, level: level.queryV.level, forecast: fcRe });
    ...
    map.on('click', (ev) => {
        ...
        if (map.getLayer('hrrr-scalar') && scalarLayer.isVisible()) {
            ...
            if (s && !Number.isNaN(s.value)) {
                ...
                if (s.missing > 0)
                    rows.push(`<div class="inspect-row"><span class="k">note</span><span>${s.missing}/4 corners missing</span></div>`);
            }
        }
        // Wind readout
        if (windLayer.isAttached() && windLayer.isVisible()) {
            ...
            if (w && Number.isFinite(w.speed)) {
                if (rows.length)
                    rows.push('<div style="height:4px"></div>');
        ...
        if (rows.length === 1)
            rows.unshift('<div class="inspect-title" style="color:#8b949e">no data at this point</div>');
